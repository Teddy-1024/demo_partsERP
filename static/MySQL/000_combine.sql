...
BEGIN
    ...
    IF v_has_filter_supplier THEN
		...
		IF EXISTS (SELECT * FROM Temp_Split TS LEFT JOIN Shop_Supplier S ON TS.substring = S.id_supplier WHERE ISNULL(S.id_supplier)) THEN 
			INSERT INTO tmp_Msg_Error (
				guid,
				code,
				msg
		...
		ELSE
			...
			FROM Shop_Supplier S
            INNER JOIN Temp_Split TS ON S.id_supplier = TS.substring
            WHERE
				(
